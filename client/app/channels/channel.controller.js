...
angular.module('movieShareApp')
  .controller('ChannelCtrl', function(channelService, $scope, socketFactory, $state, $stateParams) {
    var vm = this;
    vm.state = $state.current;
    vm.showChannel = channelService.findById($stateParams.id);
    vm.selectedChannel = vm.showChannel;
    ...
    channelService.getMe().then(function(response) {
      ...
      channelService.getChannels().then(function(response) {
        ...
        socket.on('movie:save', function(eventData) {
          ...
          var affectedChannel = vm.showChannel;
